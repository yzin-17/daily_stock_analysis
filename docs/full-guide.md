...
> - ThesisLedger Provider registry 会列出 DSA 内置的全部 11 个数据源：AKShare、efinance、腾讯财经、Tushare Pro、TickFlow、通达信（pytdx）、BaoStock、Yahoo Finance、Longbridge、Finnhub 和 Alpha Vantage。全部数据源都可按自身声明的能力参与主备路由；需要凭证的数据源从 DSA 环境读取配置，具体标的不在其市场覆盖范围内时会继续尝试备用源。
