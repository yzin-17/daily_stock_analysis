...
| `EFINANCE_EASTMONEY_COOKIE` | Optional Eastmoney login Cookie (usually containing `ct=...`) for efinance `DAILY_BAR` when the upstream requires a logged-in session or applies rate limiting; write-only injection, never commit or log it. | - | Optional |
...
> - AkShare ETF daily bars prefer Eastmoney's forward-adjusted endpoint and automatically switch to Tencent's independent forward-adjusted endpoint when Eastmoney raises an error or returns no rows. This is an internal AkShare adapter channel switch, so ThesisLedger still reports `provider=akshare` and keeps cross-provider `fallbackUsed` semantics unchanged; the response also carries `upstreamSource=tencent` so callers can display and audit the actual channel.
> - The ThesisLedger Provider registry also exposes Tencent Finance as an independent credential-free Provider for stock and ETF `DAILY_BAR` primary/fallback routing. It does not advertise real-time quote, fund NAV, or chip-summary capabilities.
> - The registry lists all 11 DSA data sources: AKShare, efinance, Tencent Finance, Tushare Pro, TickFlow, pytdx, BaoStock, Yahoo Finance, Longbridge, Finnhub, and Alpha Vantage. Every source can participate in primary/fallback routing for its declared capabilities. Credentialed sources read their configuration from the DSA environment, and requests outside a source's market coverage continue to the fallback source.
> - The ThesisLedger daily-bar facade forwards `start`, `end`, and `limit` unchanged to the provider runtime. Explicit historical ranges therefore take effect during upstream retrieval instead of filtering a default last-90-bar response. Callers requesting one year of daily bars should set `limit` no lower than the expected trading-day count; the current API maximum is 365.
